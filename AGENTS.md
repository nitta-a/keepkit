...
- `packages/keepkit` and `packages/keepkit-ui` are the publishable packages, named `@keepkit/core` and `@keepkit/ui`.
...
- `scripts/check-package.mjs` validates all publishable packages, and `scripts/check-release-version.mjs` validates release tags.
...
Other useful commands include `pnpm package:check`, `pnpm release:check -- vX.Y.Z`, `pnpm format`, and `pnpm clean`.
...
- Use `pnpm package:check` to verify all publishable packages and `pnpm release:check -- vX.Y.Z` to verify that a release tag matches all publishable package versions.
