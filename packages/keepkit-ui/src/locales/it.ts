import type { KeepUiLocaleLabels } from "./types";

export const IT_LABELS: KeepUiLocaleLabels = {
  archive: "Archivia",
  unarchive: "Rimuovi dall'archivio",
  archived: "Archiviato",
  pin: "Fissa",
  unpin: "Rimuovi fissaggio",
  pinned: "Fissato",
  collection: "Raccolta",
  allCollections: "Tutte le raccolte",
  filterCollections: "Filtra per raccolta",
  uncategorized: "Senza categoria",
  saveWithNote: "Salva con nota",
  editSavedItem: "Modifica elemento salvato",
  unsavedChanges: "Modifiche non salvate",
  save: "Salva",
  saved: "Salvato",
  remove: "Rimuovi",
  loading: "Caricamento…",
  saving: "Salvataggio…",
  syncing: "Sincronizzazione…",
  error: "Si è verificato un errore.",
  allTags: "Tutti",
  tags: "Tag",
  filterTags: "Filtra gli elementi salvati per tag",
  activeFilters: "Filtri attivi",
  clearFilters: "Cancella filtri",
  clearAllFilters: "Cancella tutti i filtri",
  resetFilters: "Ripristina i filtri iniziali",
  removeFilter: "rimuovi",
  noFilteredItems: "Nessun elemento corrispondente trovato.",
  noFilteredItemsDescription: "Modifica la ricerca o rimuovi un filtro tag.",
  emptyStorageDescription: "Salva un elemento per visualizzarlo qui.",
  note: "Nota",
  saveNote: "Salva nota",
  noItems: "Nessun elemento salvato.",
  loadingItems: "Caricamento degli elementi salvati…",
  errorItems: "Impossibile caricare gli elementi salvati.",
  search: "Cerca elementi salvati",
  sort: "Ordina elementi salvati",
  newest: "Più recenti prima",
  oldest: "Più vecchi prima",
  savedNewest: "Salvati più di recente prima",
  savedOldest: "Salvati meno di recente prima",
  updatedNewest: "Aggiornati più di recente prima",
  updatedOldest: "Aggiornati meno di recente prima",
  previousPage: "Pagina precedente",
  nextPage: "Pagina successiva",
  pagination: "Paginazione",
  page: "Pagina",
  selectItems: "Seleziona elementi salvati",
  selectedCount: "selezionati",
  deleteSelected: "Elimina selezionati",
  tagsToApply: "Tag da applicare",
  applyTags: "Applica tag",
  savedMessage: "Elemento salvato.",
  removedMessage: "Elemento rimosso.",
  restoredMessage: "Elemento ripristinato.",
  syncFailedMessage: "Sincronizzazione non riuscita.",
  stalePrunedMessage: "Elementi non disponibili rimossi.",
  undo: "Annulla",
  undoAvailable: "Elemento rimosso. Annullare?",
  selectionScope: "Ambito della selezione",
  currentPage: "Pagina corrente",
  searchResults: "Risultati di ricerca correnti",
  allItems: "Tutti gli elementi salvati",
  localVersion: "Locale",
  remoteVersion: "Server",
  updatedAt: "Aggiornato",
  statusAvailable: "Disponibile",
  noteSavedMessage: "Nota salvata.",
  exportData: "Esporta JSON",
  importData: "Importa JSON",
  importMode: "Modalità di importazione",
  merge: "Unisci",
  replace: "Sostituisci",
  importedCount: "elementi importati",
  failedCount: "elementi non riusciti",
  storageQuotaError: "Lo spazio di archiviazione è pieno. Libera spazio e riprova.",
  statusExpired: "Scaduto",
  statusRemoved: "Rimosso",
  statusDeleted: "Eliminato",
  statusPrivate: "Privato",
  statusUnknown: "Non disponibile",
  retry: "Riprova",
  removeFromList: "Rimuovi dall'elenco",
  pruneStale: "Rimuovi elementi non disponibili",
  retrySync: "Riprova sincronizzazione",
  resolveSync: "Risolvi conflitti",
  keepLocal: "Mantieni versione locale",
  useServer: "Usa versione del server",
  manualMerge: "Unione manuale",
  close: "Chiudi",
  backupRecovery: "Ripristina dal backup",
  backupRecoveryDescription: "Se i dati locali sono danneggiati, ripristina un backup JSON valido.",
  syncPending: "Ci sono modifiche in attesa di sincronizzazione.",
  syncSynced: "Tutte le modifiche sono sincronizzate.",
  syncConflict: "Alcune modifiche richiedono la risoluzione di un conflitto.",
  archiveScope: "Ambito archivio",
  archiveScopeActive: "Attivi",
  archiveScopeArchived: "Archiviati",
  archiveScopeAll: "Tutti gli elementi",
  activityNeverOpened: "Mai aperto",
  activityOpened: "Aperto",
  activityInactiveFor: "giorni inattivo",
  activityInactiveSuffix: "inattivo",
  activityBefore: "Aperto prima di",
  activityAfter: "Aperto dopo",
  undoReorder: "Annulla riordinamento",
  toolbarStart: "Azioni della raccolta",
  toolbarQuery: "Cerca e ordina",
  toolbarFilters: "Filtri",
  toolbarEnd: "Azioni aggiuntive",
  createCollection: "Create collection",
  createCollectionSubmit: "Create",
  collectionName: "Collection name",
  manageCollections: "Manage collections",
  renameCollection: "Rename",
  renameCollectionSubmit: "Save name",
  deleteCollection: "Delete",
  deleteCollectionConfirm: "Move its items to Uncategorized?",
  deleteCollectionSubmit: "Delete collection",
  cancel: "Cancel",
  collectionItem: "item",
  collectionItems: "items",
};
