...
export type KeepInvalidationPluginOptions<TMeta = Record<string, unknown>> = {
  ...
  queryKeys:
    | readonly unknown[]
    | ((context: KeepPluginContext<TMeta>) => readonly (readonly unknown[])[]);
  /** Connect this callback to queryClient.invalidateQueries or SWR mutate. */
  invalidate: (
    queryKey: readonly unknown[],
    context: KeepPluginContext<TMeta>,
  ) => void | Promise<void>;
  name?: string;
};
...
export function createKeepInvalidationPlugin<TMeta = Record<string, unknown>>(
  ...
  return {
    ...
    after: async (context) => {
      const keys =
        typeof options.queryKeys === "function" ? options.queryKeys(context) : [options.queryKeys];
